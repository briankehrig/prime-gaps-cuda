...
- `BLOCK_SIZE` represents how much sieving we do at once, per kernel. This should be as high as possible while not getting too close to the limits of the GPU's memory. It is recommended to keep this at -1, in which case it will estimate a value such that about half of the GPUs memory will be in use at any one time. (From testing, using an amount of memory close to 100% can cause performance loss.)
...
- `SMALL_PRIME_WHEELS`is the number of small prime wheels to use. (See above explanation)
- `NUM_MEDIUM_PRIMES_BASE` is (approximately) the number of primes to sieve, after applying the small prime wheels. It must be a multiple of 512, but it is recommended to keep this at -1 to automatically choose a good value.
...
- `NAME`Your name. This will get included in the gap report. It is recommended that this is exactly 8 characters long.
## Using multiple GPUs
...
## Sieving
1. We begin with a bunch of memory for a sieve, initialized to 0. Within each byte, the 8 bits represent numbers 1,7,11,13,17,19,23,29 more than a multiple of 30, respectively. In older versions of the code, every block of 30 is represented in the sieve as a bit, but a significant performance improvement was achieved when I started only sieving *half* of all blocks of 30, and leaving out the rest. For example, byte #0 might sieve x+(1,7,11,13,17,19,23,29), byte #1 sieves x+(61,67,71,73,77,79,83,89), completely skipping values from x+30 to x+60. With this alternating strategy, we only have to sieve half as many values at once, and the later post-sieving parts of the code have to do negligibly more work. This is represented by the `WORD_LENGTH` parameter, which is either 120 (sieving every byte) or 240 (skipping every 2nd byte).
